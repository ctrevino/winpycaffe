...
f = open('camvid_list.txt','r')
inputs = f.read().splitlines()
inputs = [inputs[i] for i in ridx]
f.close()

in_db = lmdb.open('D:\camvid-lmdb', map_size=int(58720256*7))
...
f = open('camvid_list_gt.txt','r')
inputs = f.read().splitlines()
inputs = [inputs[i] for i in ridx]
f.close()

in_db = lmdb.open('D:\camvid-gt-lmdb', map_size=int(20971520*7))
with in_db.begin(write=True) as in_txn:
    for in_idx, in_ in enumerate(inputs):
        im = Image.open(in_) # load image
